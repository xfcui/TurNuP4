...
str_fp_p_s,diff_fp_p_s,DRFP_p_s,ESM1b_p_s,ESM1b_ts_p_s,ESM1b_ts_DRFP_p_s,ESM1b_ts_DRFP_mean_p_s = [],[],[],[],[],[],[]
...
str_fp_M_s,diff_fp_M_s,DRFP_M_s,ESM1b_M_s,ESM1b_ts_M_s,ESM1b_ts_DRFP_M_s,ESM1b_ts_DRFP_mean_M_s = [],[],[],[],[],[],[]
...
str_fp_R_s,diff_fp_R_s,DRFP_R_s,ESM1b_R_s,ESM1b_ts_R_s,ESM1b_ts_DRFP_R_s,ESM1b_ts_DRFP_mean_R_s = [],[],[],[],[],[],[]

result_model = ['str_fp','diff_fp','DRFP','ESM1b','ESM1b_ts','ESM1b_ts_DRFP','ESM1b_ts_DRFP_mean']
labs = ['str_fp_me','str_fp_author','str_fp_same','diff_fp_me','diff_fp_author','diff_fp_same','DRFP_me','DRFP_author','DRFP_same','ESM1b_me','ESM1b_author','ESM1b_same','ESM1b_ts_me','ESM1b_ts_author','ESM1b_ts_same','ESM1b_ts_DRFP_me','ESM1b_ts_DRFP_author','ESM1b_ts_DRFP_same','ESM1b_ts_DRFP_mean_me','ESM1b_ts_DRFP_mean_author','ESM1b_ts_DRFP_mean_same']
...
for model in result_model:
    ...
    listname = model+'_'+result_y+'_s'
    with open(datadir+model+result_y+"s.txt", "r") as f:
        lines = f.readlines()
        for line in lines:
            globals()[listname].append(float(line.strip()))
...
for model in result_model:
    ...
    listname_s = model+'_'+result_y+'_s'
    result_s = globals()[listname_s]
        
    Boxplots.append(result_m)
    Boxplots.append(result_h)
    Boxplots.append(result_s)
    ticks.append(i+1)
    ticks.append(i+2)
    ticks.append(i+3)
    i = i+3
...
for i, box_data in enumerate(Boxplots):
    box_color = me_box_color if i % 3 == 0 else (old_box_color if i % 3 == 1 else same_box_color)
