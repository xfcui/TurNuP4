...
data = pd.read_pickle('./kcat/TurNuP4/data/' + "kcat_data/finish_propress.pkl")

data['2A2b2p'] = data.apply(lambda row: row['DRFP_del2'] + row['DRFP_del'] + row['DRFP'], axis=1)
# data['2A2b2p_imb'] = data.apply(lambda row: row['DRFP_del2_imb'] + row['DRFP_del_imb'] + row['DRFP_imb'], axis=1)
...
all_zero_rows = np.all(sub_only == 0.0, axis=1)
if len(all_zero_rows)>0:
    pass
all_zero_rows = np.all(pro_only == 0.0, axis=1)
if len(all_zero_rows)>0:
    pass
#确定将这部分（结构）归到哪一类
...
data.to_pickle('./kcat/TurNuP4/data/'+"kcat_data/finish_propress.pkl")
